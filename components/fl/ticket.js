...
define(function (require, exports, module) {
     ...
     * 国际出发热门城市
     */
    function initGjCfHotCity(){
        if($.fn.gjCfHotCity) return ;
        //Tab选项卡的默认配置
        var tabOpts = {
            type:2,
            typeValue:"/cdsbase/kj/cds/cityjc/gjjphotcitylist?gngj=0",
            itemWidth:70,
            tabWidth:80,
            rightWidth:50,
            simpleData:{
        ...
        var pulldownOpts = {
            dataType:"dynamic",
            type:2,
            typeValue:"/cdsbase/kj/cds/cityjc/csjclist",
            simpleData:{
                id:"nbbh",
                name:"csmc"
            },
            ...
            fn1:function(data){
                var mc = data.csmc+" "+data.szgjmc+" ("+data.nbbh+")";
                return "<p><span style='width:80%;'title='"+mc+"'>"+mc+"</span><span class='right' style='width:20%;'> </span></p>";
            }
        };
        $.fn.gjCfHotCity = function(opts1,opts2){
            var tab  =$(this).data("tabComponent"), pulldown = $(this).data("pulldownComponent");
            if(tab && pulldown){
                tab.destroy();
                pulldown.destroy();
                $(this).off(".hotCity");
            ...
            $(this).on("click.hotCity",function(){
                if(tabIsLoaded){
                    tab.show();
                }else{
                    tab = new VetechTab(this,$.extend(true,{},tabOpts,opts1));
                    tab.load(function(data){
                        var arr = [],mz = data.result.continentMap["0003"]||[];
                        mz = mz.concat(data.result.continentMap["0004"]||[]);
                        var gnrm = {
                            text:'国内热门',
                            groups:[{gName:"",items:data.result.gnHotCityList}]
                        }, yazhou ={
                            text:'亚洲',
                            groups:[{gName:"",items:data.result.continentMap["0001"]||[]}]
                        }, meizhou ={
                            text:'美洲',
                            groups:[{gName:"",items:mz}]
                        }, ouzhou ={
                            text:'欧洲',
                            groups:[{gName:"",items:data.result.continentMap["0002"]||[]}]
                        }, feizhou ={
                            text:'非洲',
                            groups:[{gName:"",items:data.result.continentMap["0005"]||[]}]
                        }, dayangzhou ={
                            text:'大洋洲',
                            groups:[{gName:"",items:data.result.continentMap["0006"]||[]}]
                        }, gjrm = {
                            text:'国际热门',
                            groups:[{gName:"",items:data.result.gjHotCityList}]
                        };
                        arr.push(gnrm,yazhou,meizhou,ouzhou,feizhou,dayangzhou,gjrm);
                        this.data = arr;
                        this.init(function(){
                            this.show();
                        });
     ...
     * 国际到达热门城市
     */
    function initGjDdHotCity(){
        if($.fn.gjDdHotCity) return ;
        //Tab选项卡的默认配置
        var tabOpts = {
            type:2,
            typeValue:"/cdsbase/kj/cds/cityjc/gjjphotcitylist?gngj=0",
        ...
        $.fn.gjDdHotCity = function(opts1,opts2){
            var tab  =$(this).data("tabComponent"), pulldown = $(this).data("pulldownComponent");
            if(tab && pulldown){
                tab.destroy();
                pulldown.destroy();
            ...
            $(this).on("click.hotCity",function(){
                if(tabIsLoaded){
                    ...
                    tab.load(function(data){
                        ...
                        arr.push(gjrm,yazhou,meizhou,ouzhou,feizhou,dayangzhou,gnrm);
                        this.data = arr;
                        this.init(function(){
                            this.show();
                        });
     ...
     *初始化国内国际城市机场下拉列表
     */
    function initGnGjCsJc(){
        if($.fn.addGnGjCsJc) return;

        $.fn.addGnGjCsJc = function(opts){
            ...
            var options = $.extend(true,{
                ...
                fn1:function(data){
                    if(data.sfcs=="1"){
                        return "<span>"+data.csmc+"("+( data.hxid || "") +")</span><lable class='layui-fl-right iconfont icon-city'></lable>";
                    }else{
                        if(data.pid){
                            return "<lable class='layui-fl-left layui-pl5 iconfont icon-gwjt'></lable><span>"+data.jcmc+"("+( data.nbbh || "") +")</span>";
                        }else{
                            return "<span>"+data.jcmc+"("+( data.nbbh || "") +")</span><lable class='layui-fl-right iconfont icon-jichang'></lable>";
                        }
                    }
                },
                type:2,
                typeValue:"/cdsbase/kj/cds/cityjc/csjclistpage",
                "dataType":"dynamic"
            },opts);
            var pulldown = new Pulldown(this.get(0), options);
    ...
    exports.initGjCfHotCity = initGjCfHotCity;

    exports.initGjDdHotCity = initGjDdHotCity;

    exports.initGnGjCsJc = initGnGjCsJc;
