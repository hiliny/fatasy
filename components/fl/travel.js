...
define(function (require, exports, module) {
    var Pulldown = require("js/Pulldown"),
        ...
        PriceCalender =  require("js/PriceCalender"),
        Gjz = require("js/Gjz"),
     ...
     * 单价格日历控件
     */
    function initPriceCalender(){
        if($.fn.addPriceCalender) return;
        $.fn.addPriceCalender = function(options){
            //如果有重复绑定的情况，先清除
            if(this.data("component")){
                this.data("component").destroy();
                this.off(".pricecalender");
            }
            var tempOpts = $.extend({},{
                type:2,
                typeValue:"/cds-mp/cpsb/JqYd/getJgrl"
            },options);
            var pc = new PriceCalender(this.get(0),tempOpts);
            this.data("component",pc);
            pc.load(function(data){
                this.data = data.result.records || data.result;
                this.render();
            });
            if(tempOpts.usetype==="append"){
                pc.show();
            }else{
                this.on("click.pricecalender",function(){
                    pc.show();
                });
            }
        }
    ...
    exports.initPriceCalender = initPriceCalender;
